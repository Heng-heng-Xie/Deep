...
class Match:
    ...
    def run(self, team1, team2, num_player=1, max_frames=MAX_FRAMES, max_score=3, record_fn=None, timeout=1e10,
        ...
        #print(race.render_data)

        save_loc = 'training_data/train'

        import os
        from PIL import Image
        destination = pathlib.Path(save_loc)
        ...
        for it in range(max_frames):
            ...
            if self._use_graphics:
                ...
                for i in range(4):
                    ...
                    Image.fromarray(image).save(f"{destination}/images/{i+4}_{it}.png")

                    mask = race.render_data[i].instance == 134217729
                    Image.fromarray(mask).save(f"{destination}/labels/{i+4}_{it}.png")
            ...
            #for i in range(len(race.render_data)):
                    #image = race.render_data[i].image
                    #Image.fromarray(image).save(f"{destination}/images/{i}_{it}.png")
                    #mask = race.render_data[i].instance == 134217729
                    #Image.fromarray(mask).save(f"{destination}/masks/{i}_{it}.png")
